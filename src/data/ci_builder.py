...
class CIExperience:
    ...
    def make_loaders(self, batch_size: int, num_workers: int = 8, shuffle_train: bool = True):
        train_loader = DataLoader(self.train_ds, batch_size=batch_size, shuffle=shuffle_train,
...
class CIScenario:
    ...
    def __len__(self): return len(self._exps)
    def __getitem__(self, i) -> CIExperience: return self._exps[i]

    @property
    def train_stream(self): return self._exps
    @property
    def valid_stream(self): return self._exps
    @property
    def test_stream(self):  return self._exps

def _split_classes_into_experiences(all_class_ids: Sequence[int], n_experiences: int, scenario: int = 2) -> List[List[int]]:
    """
...
def _subset_by_classes(data_np: np.ndarray, keep_class_ids: Sequence[int]) -> np.ndarray:
...
def _get_benign_split_indices(all_data: np.ndarray, benign_class: int = 0, 
                             exp_id: int = 0, n_experiences: int = 4) -> np.ndarray:
    """
...
def _subset_by_classes_with_scenario(data_np: np.ndarray, keep_class_ids: Sequence[int], 
                                   scenario: int = 2, benign_class: int = 0, 
                                   exp_id: int = 0, n_experiences: int = 4, 
                                   benign_indices: np.ndarray = None) -> np.ndarray:
    """
    ...
    non_benign_mask = np.isin(labels, np.array([c for c in keep_class_ids if c != benign_class], dtype=np.int64))
...
def build_class_incremental_scenario(
    ...
    n_experiences: int,
    class_order: Optional[Sequence[int]] = None,  # order of global class ids; default = [0..C-1]
    ...
    if class_order is None:
        ...
        assert len(set(class_order)) == num_class, "class_order must include each class exactly once"
    ...
    if scenario == 2:
        for exp_id, cls_ids in enumerate(class_groups):
            ...
            benign_indices_train = _get_benign_split_indices(train_np, benign_class=0, exp_id=exp_id, n_experiences=n_experiences)
            benign_indices_val = _get_benign_split_indices(val_np, benign_class=0, exp_id=exp_id, n_experiences=n_experiences)
            benign_indices_test = _get_benign_split_indices(test_np, benign_class=0, exp_id=exp_id, n_experiences=n_experiences)

            # slice each split by class set, with benign samples split across experiences
            tr_np = _subset_by_classes_with_scenario(train_np, cls_ids, scenario, benign_class=0, exp_id=exp_id, n_experiences=n_experiences, benign_indices=benign_indices_train)
            va_np = _subset_by_classes_with_scenario(val_np, cls_ids, scenario, benign_class=0, exp_id=exp_id, n_experiences=n_experiences, benign_indices=benign_indices_val)
            te_np = _subset_by_classes_with_scenario(test_np, cls_ids, scenario, benign_class=0, exp_id=exp_id, n_experiences=n_experiences, benign_indices=benign_indices_test)

            # Build train TabularDataset (fit=True → creates scaler), then reuse that scaler
            train_ds = TabularDataset(tr_np, categorical_indices_file,
                                      task_classes=cls_names, all_classes=all_classes,
                                      fit=True, scaler=None)
            val_ds   = TabularDataset(va_np, categorical_indices_file,
                                      task_classes=cls_names, all_classes=all_classes,
                                      fit=False, scaler=train_ds.scaler)
            test_ds  = TabularDataset(te_np, categorical_indices_file,
                                      task_classes=cls_names, all_classes=all_classes,
                                      fit=False, scaler=train_ds.scaler)

            exps.append(CIExperience(
                exp_id=exp_id, class_ids=list(cls_ids), class_names=cls_names,
        ...
        for exp_id, cls_ids in enumerate(class_groups):
            ...
            tr_np = _subset_by_classes_with_scenario(train_np, cls_ids, scenario, benign_class=0, exp_id=exp_id)
            va_np = _subset_by_classes_with_scenario(val_np, cls_ids, scenario, benign_class=0, exp_id=exp_id)
            te_np = _subset_by_classes_with_scenario(test_np, cls_ids, scenario, benign_class=0, exp_id=exp_id)
...
def build_dataset_incremental_scenario(
    ...
    if class_order is None:
        ...
        class_order = list(class_order)
        assert len(set(class_order)) == num_class_total, "class_order must include each class exactly once"
    ...
    UNSW_train_np, UNSW_val_np, UNSW_test_np, class_groups = remap_labels(UNSW_train_np, UNSW_val_np, UNSW_test_np, all_classes, num_class_1)
    ...
    exp = [
        ...
        # exp 1 (UNSW)
        (1, UNSW_cat_idx_file, UNSW_train_np, UNSW_val_np, UNSW_test_np, class_groups[1])
    ]
